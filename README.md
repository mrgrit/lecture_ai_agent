...
실습편 C의 명령에는 `python3` 가 85번 나오고 그중 44번이 `python3 -c "..."` 인라인 코드입니다.
검증에는 이 방식이 가장 좋지만, 수업에서는 **에이전트를 배우러 온 학생이 JSON 파싱 코드를 해석하게 됩니다.**
그리고 더 근본적으로, `claude -p "..." --output-format stream-json --verbose > x.jsonl` 은
**기계가 쓰는 방식이지 사람이 쓰는 방식이 아닙니다.** 실제 학생은 `claude` 를 띄우고 프롬프트창에 말을 넣습니다.

실습편 S는 같은 스물세 개를 **사람이 하는 그대로** 다시 썼습니다.

- 모델에게 시키는 일 → **창을 열고 프롬프트를 친다**
- 결과 확인 → **화면의 `● Skill(...)` 줄을 읽거나 `cat` 으로 파일을 연다**
- `/skills` `/mcp` `/hooks` `/permissions` `/memory` `/cost` `/context` `/resume` 를 실제로 쓴다
- 만드는 프로그램은 파이썬 대신 셸 (`slugify.sh`, `duration.sh`)
- MCP 커넥터는 코드를 주지 않고 **클로드코드에게 명세를 주고 만들게 함**
- `claude -p` 는 딱 한 자리에만 남겼습니다 — **루프**. 사람이 없을 때 도는 것이니 거기서는 그게 맞고,
  그 구분(창이냐 `-p` 냐) 자체가 실습 내용이 됩니다.

C 트랙은 지우지 않고 그대로 두었습니다. 강사가 수업 전에 환경을 한 번에 점검할 때,
학생이 막혔을 때 정답 전체를 볼 때, 같은 일을 파이썬으로 어떻게 하는지 비교할 때 필요하기 때문입니다.
두 트랙의 실습 카드에서 서로 오갈 수 있습니다.
...
- `src/verify-labs/` — 실습편 C·S의 검증 하니스
  - `extract_cc.py` · `runcc.py` — 실습편 C (셸 블록을 그대로 실행)
  - `extract_s.py` · `runs.py` — 실습편 S (창에 칠 말을 `claude -p` 로 옮기고 `--continue` 로 잇는다)
  - `probe.py` — 세션 기록에서 도구 호출·발동 스킬·답변을 뽑는 판정용 조회기
  - `vt.py` · `drive.py` — 대화창을 유사 터미널로 띄우고 화면을 재구성하는 최소 터미널 모형
  - `capture.py` · `paste_caps.py` — 문서의 '화면에 나오는 것' 블록을 실제 캡처로 채운다

## 실습편 C·S의 검증 방법

두 트랙 모두 문서에 적힌 명령을 그대로 실행하고,
실습마다 "무엇이 참이면 성공인가"를 기계가 검사하는 방식으로 확인했습니다.

**실습편 S는 문서가 "창을 열고 프롬프트를 쳐라"로 적혀 있어서 검증 방식이 다릅니다.**

| 문서의 블록 | 검증할 때 |
|---|---|
| 터미널 명령 | 그대로 실행 |
| 창에 칠 말 | 같은 문구를 `claude -p` 로 전송. 창 안에서 이어 친 말은 `--continue` 로 이어 한 대화로 만든다 |
| 슬래시 명령 | 사람만 할 수 있으므로 건너뜀 |
| 화면에 나오는 것 | **실제 대화창을 유사 터미널(pty)로 띄워 캡처** — 지어낸 화면 없음 |
| 판정 | 만들어진 파일과 세션 기록(`~/.claude/projects/`) |

**프롬프트 문구는 한 글자도 바꾸지 않습니다.** 문서의 말이 그대로 모델에게 가고, 다른 것은 전달 방식뿐입니다.
...
python3 extract_s.py  && python3 runs.py    # 실습편 S — 문서를 실행 가능한 형태로 옮겨 24개 구간

python3 capture.py                          # 화면 캡처 (대화창을 실제로 띄운다)
python3 paste_caps.py                       # 캡처를 문서에 끼워 넣는다

rm shome/.claude && rm -rf shome            # 링크를 먼저 지운다
```

> 스크립트는 실제로 모델을 호출하므로 요금이 발생합니다.

두 트랙 모두 **완전히 빈 환경에서 처음부터 끝까지 여러 번** 돌려, 마지막 회차에 24개 구간 전부 통과하는 것을 확인했습니다.

| | 실습편 C | 실습편 S |
|---|---|---|
| 문서의 블록 | bash 94개 | 터미널 208 · 창에 칠 말 69 · 화면 22 |
| 검증 회차 | 3회 | 5회 |
| 마지막 회차 | 24/24 · 14분 | 24/24 · 14분 |
| 모델 대화 | 44회 | 42회 |
| 1회 통과 비용 (목록가) | 약 $2.5 | 약 $2.6 |
| 모델을 안 부르는 실습 | `C0-1` `C3-1` `C4-2` | `S0-1` `S3-1` |
...
실습편 S에서 새로 나온 것: 승인해 줄 사람이 없는데 에이전트가 승인을 기다린 일(`S2-4`·`S5-1`·`S5-3`·`S5-4`),
폴더 규칙의 첫 줄이 슬래시 명령의 첫 줄을 밀어낸 일(`S5-2`), 서브에이전트가 절을 덧붙이며 마커를 흘린 일(`S2-5`),
계획 모드가 빈 폴더를 몇 분씩 뒤진 일(`S5-1`), `work/` 규칙 때문에 파일이 엉뚱한 곳에 생긴 일(`S5-3`),
`cut -c` 가 한글을 바이트로 자른 일(`S2-2`), 그리고 **창에서 이어 친 말이 새 대화로 전달되어 "방금"이 사라진 일**(`S1-1`).
