...
def _who():
    u, o = "s-who-does-what", []
    o.append(hdr(360, 16, "어디에 치는 것인가 — 창과 터미널"))

    cols = [("대화창에 친다", "d-ok",
             ["일을 시키는 프롬프트", "/skills /mcp /hooks", "/cost /context /memory",
              "화면의 ● 줄을 읽는다", "승인 화면에서 고른다"]),
            ("터미널에 친다", "d-accent",
             ["폴더와 파일 만들기", "설정·스킬·훅 파일 쓰기", "cat 으로 결과 열기",
              "훅을 단독으로 시험", "루프 스크립트 (claude -p)"]),
            ("이 트랙에 없는 것", "d-warn",
             ["stream-json 으로 받기", "JSON 을 정규식으로 긁기",
              "채점 스크립트 해석하기", "— 사람이 하는 일이 아니다"])]
    cw, gap = 228, 18
    ...
    o.append(box(0, 212, 350, 62, "사람이 앞에 있으면 창",
                 ["승인 화면이 뜨고, 사람이 고른다.", "실습 0~2 · 4~5 가 여기다"],
                 cls="d-ok", tsize=11.5))
    o.append(box(370, 212, 350, 62, "사람이 없으면 -p",
                 ["승인 화면이 뜰 수 없다 → ask 는 거부.", "실습 3 의 루프가 여기다"],
                 cls="d-accent", tsize=11.5))
    return "".join(o), 286


reg("s-who-does-what", W, _who()[1],
    "블록이 어디에 치는 것인지에 따라 세 갈래로 나뉜다. 대화창에 치는 것은 일을 시키는 프롬프트와 슬래시 명령이고, 화면의 도구 호출 줄을 읽고 승인 화면에서 고르는 것도 창에서 한다. 터미널에 치는 것은 폴더와 파일 만들기, 설정과 스킬과 훅 파일 쓰기, cat 으로 결과 열기, 훅을 단독으로 시험하기, 그리고 루프 스크립트다. 이 트랙에 없는 것은 stream-json 으로 받기, JSON 을 정규식으로 긁기, 채점 스크립트 해석하기이며 그것은 사람이 하는 일이 아니기 때문이다. 사람이 앞에 있으면 창을 쓰고 승인 화면에서 고르며, 사람이 없으면 비대화 모드를 쓰고 승인 화면이 뜰 수 없으므로 ask 규칙은 전부 거부된다.",
    _who()[0])
